# Used to generate a Snowflake JWT token for authenticating with Snowflake.
...
class JWTGenerator(object):
    ...
    def get_token(self) -> Text:
        ...
        if self.token is None or self.renew_time <= now:
            logger.debug("Generating a new token because the present time (%s) is later than the renewal time (%s)",
                        now, self.renew_time)
            ...
            self.token = token
            logger.debug("Generated a JWT with the following payload: %s", jwt.decode(self.token, key=self.private_key.public_key(), algorithms=[JWTGenerator.ALGORITHM]))
